...
class TestRunner(object):
    ...
    def do_pcdiffs(self, diff_exec, test_, g_maps):
        for g_map in g_maps:
            ...
            #values = reader_.getValues()
            #too_diff=False
            #for (x,y),val in np.ndenumerate(values):
            #        if val!=reader_.getMissingValue() and abs(val)>eps:
            #            print str(abs(val))
            #            too_diff=True
            #if too_diff:
            #    print '[ERROR] values too different!'
            #else:
            #    print '[GOOD] values are good!'

    def run(self):
        #print self._ctx.get('tests')
        diff_exec = self._ctx.get('pcrasterdiff.exec')
        #eps = self._ctx.get('eps')
        ...
        for key_, test_ in ordered_tests.iteritems():
            ...
            if test_.g2p_command:
                ...
                elapsed = time.time()-a
                print BOLD+'grib2pcraster test '+test_.id+' executed in '+str(datetime.timedelta(seconds=elapsed))+ENDC
            ...
            if test_.g2p_command:
                if p_num_maps != g_num_maps:
                    raw_input(FAIL+BOLD+'xxxxxxx! ATTENTION!!! Potential bug! Number of maps are different p:' + str(p_num_maps)+' g:' + str(g_num_maps)+ENDC)
