...
class Corrector(object):
    ...
    def correct(self, values):
        ...
        with np.errstate(over='ignore'):
            values_corrected = self._f(values, self._dem_values, self._gem_values, self._dem_missing_value)
    ...
    def _readGeopotential(self, gribFile, interpolator, is_grib_interpolation):
        ...
        values_corrected = self._fgem(values, missing)

        if is_grib_interpolation:
            values_resampled = interpolator.interpolate_grib(values_corrected, -1, messages.getGridId())
        else:
