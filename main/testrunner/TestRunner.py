...
class TestRunner(object):
    ...
    def do_pcdiffs(self, diff_exec, test_, g_maps):
        ...
        for g_map in g_maps:
            ...
            if all_ok:
                print_colored(GREEN, '[GOOD] All values are good!')
            else:
                ...
                if perc_wrong > 5:  # more than 5% of differences
                    failed = True
                    print_colored(FAIL, '[ERROR] {:3.4f}% of values are too different!'.format(perc_wrong))
                elif 0.5 <= perc_wrong <= 5:
                    problematic = True
                    print_colored(WARN, '[WARN] values are very similar but with {:3.4f}% of differences!'.format(perc_wrong))
                else:
                    print_colored(GREEN, '[GOOD] Almost all values are good [{:3.4f}% of differences]!'.format(perc_wrong))
        if failed:
            return '1'
        elif problematic:
            return '2'
    ...
    def _print_time_diffs(elapsed_counter_part, elapsed_pyg2p, test_, from_scipy=False):
        ...
        print_colored(color_code, 'Difference: {}'.format(str(datetime.timedelta(seconds=differ_elaps))))
    ...
    def print_test_summary(self, avg_mem, avg_mem_scipy, elapsed_g2p, elapsed_pyg2p, elapsed_pyg2p_scipy, max_mem,
        ...
        print_colored(YELLOW, 'pyg2p memory usage: max {:6.2f}MB, avg {:6.2f}MB '.format(max_mem, avg_mem))
        if elapsed_g2p:
            ...
            print_colored(YELLOW, 'pyg2p memory usage with scipy: max {:6.2f}MB, avg {:6.2f}MB '.format(max_mem_scipy, avg_mem_scipy))
    ...
    def print_test_suite_summary(elapsed_test, num_tests, results):
        ...
        print_colored(YELLOW, '\n\n\n=========== TEST SUITE SUMMARY ==============')
        print_colored(YELLOW, '{} tests  executed in {}'.format(str(num_tests), str(datetime.timedelta(seconds=elapsed_test))))
        print_colored(YELLOW, 'successful {}, problematic: {}, failed: {}'.format(len(results['0']), len(results['2']), len(results['1'])))
        print_colored(FAIL, 'Failed tests: {}'.format(str(results['1'])))
        print_colored(WARN, 'Problematic tests: {}'.format(str(results['2'])))
        print_colored(GREEN, 'Successful tests: {}'.format(str(results['0'])))
        print_colored(YELLOW, '\n\n=================== END ======================\n')
    ...
    def _check_maps(self, p_num_maps, test_, o_num_maps, o_maps, results):
        if p_num_maps != o_num_maps:
            print_colored(FAIL, 'xxxxxxx! ATTENTION!!! Potential misconfiguration or bug!')
            print_colored(FAIL, 'Number of maps are different p: {} g: {}'.format(p_num_maps, o_num_maps))
